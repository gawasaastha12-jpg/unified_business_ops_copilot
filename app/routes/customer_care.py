...
@router.post("/customer-care/{event_id}")
def process_customer_care_event(event_id: int, db: Session = Depends(get_db)):
    event = db.query(models.Event).filter(models.Event.id == event_id).first()
    if not event:
        raise HTTPException(status_code=404, detail="Event not found")
        
    if event.domain != "customer_care":
        raise HTTPException(status_code=400, detail="Event is not routed to customer_care domain. Route it first.")
